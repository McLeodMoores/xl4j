...
public class ReflectiveInvokerFactoryTest {
  ...
  public void testFieldInvokerType() {
    FieldInvoker invoker = FACTORY.getFieldTypeConverter(INT_FIELD, TypeConversionMode.OBJECT_RESULT);
    assertTrue(invoker instanceof ObjectFieldInvoker);
    invoker.invoke(null); // will fail if there's an issue getting the field
    invoker = FACTORY.getFieldTypeConverter(XL_VALUE_FIELD, TypeConversionMode.PASSTHROUGH);
    assertTrue(invoker instanceof PassthroughFieldInvoker);
    invoker.invoke(null); // will fail if there's an issue getting the field
