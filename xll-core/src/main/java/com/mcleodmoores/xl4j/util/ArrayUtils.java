...
public class ArrayUtils {
   ...
   */
  public static <T> T[][] transposeIfNeeded(T[][] arr, int fixedDimensionSize, FixedDimension fixedDimension) throws ArrayWrongSizeException {
    ArgumentChecker.notNull(arr, "arr");
    ...
    if (fixedDimension == FixedDimension.ROWS) {
      if (arr.length == fixedDimensionSize) {
        ...
        throw new ArrayWrongSizeException("Array needed to be of size " + fixedDimensionSize + " in at least one dimension, but was " + arr.length + " x " + arr[0].length);
      }
    } else /*if (fixedDimension == FixedDimension.COLUMNS)*/ { // we already ruled out null...
