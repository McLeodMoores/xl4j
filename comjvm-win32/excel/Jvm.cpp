...
Jvm::Jvm () : m_lRefCount (1) {
	...
	HRESULT hr = CoCreateInstance(CLSID_JvmSupport, NULL, /*CLSCTX_INPROC_SERVER*/CLSCTX_LOCAL_SERVER, IID_IJvmSupport, (LPVOID *)&pJvmSupport);
	if (FAILED(hr)) {
		_com_error err(hr);
		LOGERROR("CoCreateInstance of JvmSupport object failed: %s", err.ErrorMessage());
		_com_raise_error(hr);
	} else {
		LOGTRACE("CoCreateInstance succeeded");
	}
	...
	hr = pJvmSupport->CreateTemplate(TEXT("default"), &pTemplate);
	if (FAILED (hr)) {
	...
	pTemplate->put_Type(SysAllocString(_T("ComJvmLocal.LocalJvmConnector.1")));
	IJvmContainer *pJvmContainer;
	hr = pJvmSupport->Connect(_bstr_t(L""), pTemplate, &pJvmContainer);
	if (FAILED(hr)) {
		_com_error err(hr);
		LOGERROR("Could not connect to JVM: %s", err.ErrorMessage());
		_com_raise_error(hr);
	}
	hr = pJvmContainer->Jvm(&m_pJvm);
	if (FAILED(hr)) {
		_com_error err(hr);
		LOGERROR("Could not get JVM from container: %s", err.ErrorMessage());
		_com_raise_error(hr);
	}
	//// Load settings.
